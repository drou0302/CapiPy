...
def main(folder_name, monomeric_model, multimeric_model):
...
# Locate the workspace#
    ...
    def superimpose_chains(dict):    
        ...
        if "linux" in platform:
            ...
            if os.path.isfile(clustalw_exe) is False:
                clustalw_exe = ""
                clustalw_exe = input("Please indicate where to find clustalw2.exe.\n").replace("/", "\\")
    ...
    if number_of_chains > 1:
        ...
        if 2 not in checks.values():
            with open('quaternary_assembly.pdb', 'w+') as fullquerymodel:  
    ...
    os.rmdir("obsolete")
    keep = ["quaternary_assembly.pdb", "query_multimeric.pdb", "query_monomer.pdb", "blast_result.xml", pdbfile,
            "query.fasta", "model.fasta"]
    files = []
    for f in os.listdir("."):
        if os.path.isfile:
            files.append(f)
