...
def main(folder_answer, folder_name, prot_sequence, blast_response):
    ...
    keep = ["query_multimeric.pdb", "query_monomer.pdb", "blast_result.xml", pdbfile, "query.fasta", "model.fasta"]
    files = []
    for f in os.listdir("."):
        if os.path.isfile:
            files.append(f)
